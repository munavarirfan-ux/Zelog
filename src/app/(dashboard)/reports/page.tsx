...
function SummaryTab({ entries }: { entries: TimeEntry[] }) {
  ...
  const maxDaily = chartView === "billability"
    ? Math.max(...dailyData.map((d) => d.billable + d.nonBillable), 1)
    : Math.max(...dailyByProject.map((d) => d.total), 1);
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-card border border-border/[0.07] bg-surface p-5 shadow-card dark:border-white/[0.06]">
        ...
        <div className="flex items-end gap-1.5 h-44 transition-all duration-300">
          {chartView === "billability" ? (
            dailyData.map((day) => {
              const totalH = (day.billable + day.nonBillable) / 3600;
              const pct = (day.billable + day.nonBillable) / maxDaily;
              const billablePct = day.billable / (day.billable + day.nonBillable || 1);
              return (
                <div key={day.date} className="group/bar relative flex flex-1 flex-col items-center gap-1">
                  <div className="relative w-full flex flex-col justify-end" style={{ height: "140px" }}>
                    <div
                      className="w-full rounded-t-md bg-accent/20 transition-all duration-200 group-hover/bar:bg-accent/30"
